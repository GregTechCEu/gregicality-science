...
public class MetaTileEntityLargeRocketEngine extends FueledMultiblockController {
    ...
    public MetaTileEntityLargeRocketEngine(ResourceLocation metaTileEntityId) {
        super(metaTileEntityId, GCYSciRecipeMaps.ROCKET_FUEL_RECIPES, GTValues.V[GTValues.EV]);
    }
    ...
    protected void addDisplayText(List<ITextComponent> textList) {
        ...
        if (isStructureFormed()) {
            FluidStack hydrogen = importFluidHandler.drain(GCYSciMaterials.LiquidHydrogen.getFluid(Integer.MAX_VALUE), false);
    ...
    public static class RocketEngineWorkableHandler extends FuelRecipeLogic {
        ...
        private boolean checkBoost() {
            FluidStack hydrogenStack = fluidTank.get().drain(GCYSciMaterials.LiquidOxygen.getFluid(oxygenNeededToBoost), false);
        ...
        protected long startRecipe(FuelRecipe recipe, int fuelUsed, int recipeDuration) {
            ...
            if (checkBoost()) {
                fluidTank.get().drain(GCYSciMaterials.LiquidOxygen.getFluid(oxygenNeededToBoost), true);
                EUt *= 3;
            }
