...
public class PressureMultiblockRecipeLogic extends MultiblockRecipeLogic {
    ...
    protected boolean drawPressure(double pressure, boolean simulate) {
        ...
        if (pressure != GCYSValues.EAP) pressureToChange = containerPressure * 0.01;
        else return true;

        if (pressure > GCYSValues.EAP) pressureToChange = -pressureToChange;
    ...
    protected IPressureContainer getPressureContainer() {
        return ((IPressureMultiblock) this.metaTileEntity).getPressureContainer();
    }
